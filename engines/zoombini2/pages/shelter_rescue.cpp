...
RescuePage::RescuePage(Zoombini2Engine *engine, int rescueNum)
	: ShelterPage(engine), _rescueNum(rescueNum),
...
void RescuePage::init() {
	...
	// Load the site background through the installed-resource search order.
	_engine->getScreen()->fillRect(Common::Rect(kScreenWidth, kScreenHeight), 0);
	BitBlock bg;
	if (bg.load(Common::Path(basePath + "background")))
