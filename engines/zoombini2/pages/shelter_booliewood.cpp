...
BooliewoodPage::BooliewoodPage(Zoombini2Engine *engine)
	: ShelterPage(engine), _musicId(-1), _background(nullptr), _zoombiniGfx(nullptr) {
...
void BooliewoodPage::init() {
	...
	// The community uses the same music throughout its development.
	SoundManager *snd = _engine->getSoundManager();
	if (snd) {
...
void BooliewoodPage::draw(Graphics::ManagedSurface *screen) {
	...
	if (!_zoombiniGfx || _engine->_globalZoombinis.empty())
		return;

	const byte (*lut)[256] = _engine->getAlphaLUT();
	int numToDraw = MIN(static_cast<int>(_engine->_globalZoombinis.size()), kNumSeats);

	for (int i = 0; i < numToDraw; i++) {
		const Zoombini *z = _engine->_globalZoombinis[i];
