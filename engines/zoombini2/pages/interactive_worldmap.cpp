...
WorldMapPage::WorldMapPage(Zoombini2Engine *engine, WorldMapMode mode)
	: InteractivePage(engine), _mode(mode), _hoveredIcon(-1),
...
void WorldMapPage::init() {
	...
	if (!isPracticeMode() && _engine->_isSavedGame)
		_engine->writeGameSave(gs->_playerName);
	_engine->clearGlobalZoombinis();
	_engine->_isSavedGame = !isPracticeMode();
...
void WorldMapPage::openQuitDialog() {
	...
	_quitDialogBackground->copyFrom(*_engine->getCurrentScreen());
...
void WorldMapPage::closeQuitDialog() {
	...
	_quitDialogButtonHover = 0;
	if (_quitDialogBackground)
		_engine->getCurrentScreen()->copyFrom(*_quitDialogBackground);
