...
static const uint32 kFerryDefaultPerfectClearNodeCap = 1000000;
static const uint32 kFerryDefaultPerfectClearTimeCapMs = 1000;
static const uint32 kFerryPerfectClearMemoCap = 200000;

enum FerryPerfectClearStatus {
	kFerryPerfectClearPossible,
	kFerryPerfectClearImpossible,
	kFerryPerfectClearTimedOut
};

struct FerryPerfectClearResult {
	FerryPerfectClearStatus _status = kFerryPerfectClearImpossible;
	uint32 _nodesVisited = 0;
	uint32 _elapsedMs = 0;
	int16 _seatToItem[20] = {};

	FerryPerfectClearResult() {
		for (int16 i = 0; i < 20; i++)
			_seatToItem[i] = -1;
	}
};

static bool ferryTraitsShareAny(const ZmbTrait &left, const ZmbTrait &right) {
	return left._head == right._head ||
		left._eye == right._eye ||
		left._nose == right._nose ||
		left._foot == right._foot;
}

static bool ferryTraitsSame(const ZmbTrait &left, const ZmbTrait &right) {
	return left._head == right._head &&
		left._eye == right._eye &&
		left._nose == right._nose &&
		left._foot == right._foot;
...
static int16 ferryCountBits(uint32 value) {
	int16 count = 0;
	while (value != 0) {
		value &= value - 1;
		count++;
...
class FerryPerfectClearSolver {
public:
	FerryPerfectClearSolver(MohawkEngine_Zoombini *vm, const byte adjacencyMatrix[20][8],
	                        const Common::Array<const ZmbSnoid *> &snoids, int16 seatCount,
	                        uint32 nodeCap, uint32 timeCapMs) :
			_vm(vm),
			_adjacencyMatrix(adjacencyMatrix),
			_snoids(snoids),
			_seatCount(seatCount),
			_snoidCount(static_cast<int16>(snoids.size())),
			_itemCount(seatCount),
			_emptyCount(static_cast<int16>(seatCount - static_cast<int16>(snoids.size()))),
			_nodeCap(nodeCap == 0 ? 1 : nodeCap),
			_timeCapMs(timeCapMs == 0 ? 1 : timeCapMs),
			_startTime(vm->_system->getMillis()) {
		for (int16 i = 0; i < 20; i++) {
			_seatToItem[i] = -1;
			_domain[i] = 0;
			_seatNeighborMask[i] = 0;
			_neighborMask[i] = 0;
			_compatSnoidMask[i] = 0;
			_classId[i] = -1;
			for (int16 j = 0; j < 20; j++)
				_share[i][j] = false;
		}

		_allItemsMask = (_itemCount >= 32) ? 0xFFFFFFFFu : ((1u << _itemCount) - 1);
		_snoidMask = (_snoidCount >= 32) ? 0xFFFFFFFFu : ((1u << _snoidCount) - 1);
		_emptyMask = _allItemsMask & ~_snoidMask;
		_unusedItemsMask = _allItemsMask;

		for (int16 i = 0; i < _snoidCount; i++) {
			for (int16 j = 0; j < _snoidCount; j++)
				_share[i][j] = ferryTraitsShareAny(_snoids[i]->_trait, _snoids[j]->_trait);
		}

		// Seat adjacency stored as a bitmask over seats for fast neighbor scans.
		for (int16 seat = 0; seat < _seatCount; seat++) {
			for (int16 slot = 0; slot < 8; slot++) {
				if (_adjacencyMatrix[seat][slot] == 0)
					continue;
				int16 neighborSeat = _adjacencyMatrix[seat][slot] - 1;
				_seatNeighborMask[seat] |= (1u << neighborSeat);
			}
		}

		// Symmetry classes: identical-trait snoids share a class; all empties form one class.
		int16 nextClass = 0;
		for (int16 i = 0; i < _snoidCount; i++) {
			for (int16 j = 0; j < i; j++) {
				if (ferryTraitsSame(_snoids[i]->_trait, _snoids[j]->_trait)) {
					_classId[i] = _classId[j];
					break;
				}
			}
			if (_classId[i] == -1)
				_classId[i] = nextClass++;
		}
		if (_emptyCount > 0) {
			int16 emptyClass = nextClass++;
			for (int16 item = _snoidCount; item < _itemCount; item++)
				_classId[item] = emptyClass;
		}

		// Allowed-neighbor (item bitmask) and compatible-snoid bitmask per occupied item.
		// An occupied snoid may sit next to any empty or any trait-sharing snoid.
		for (int16 i = 0; i < _snoidCount; i++) {
			uint32 neighborMask = _emptyMask;
			uint32 compatMask = 0;
			for (int16 j = 0; j < _snoidCount; j++) {
				if (_share[i][j]) {
					neighborMask |= (1u << j);
			...
			_neighborMask[i] = neighborMask;
			_compatSnoidMask[i] = compatMask;
		}
	}

	FerryPerfectClearResult run() {
		_failedStates.clear();
		for (int16 seat = 0; seat < _seatCount; seat++) {
			_seatToItem[seat] = -1;
			_domain[seat] = _allItemsMask;
		}
		_unusedItemsMask = _allItemsMask;

		if (failsComponentBound()) {
			_result._status = kFerryPerfectClearImpossible;
			_result._elapsedMs = _vm->_system->getMillis() - _startTime;
			return _result;
		}

		bool possible = search(0);
		_result._nodesVisited = _nodesVisited;
		_result._elapsedMs = _vm->_system->getMillis() - _startTime;
		if (possible)
			_result._status = kFerryPerfectClearPossible;
		else if (_timedOut)
			_result._status = kFerryPerfectClearTimedOut;
		else
			_result._status = kFerryPerfectClearImpossible;
		return _result;
	}
...
private:
	bool search(int16 assignedCount) {
		if (isBudgetExhausted())
			return false;
		_nodesVisited++;

		if (assignedCount == _seatCount) {
			for (int16 i = 0; i < _seatCount; i++)
				_result._seatToItem[i] = _seatToItem[i];
			return true;
		}

		// Failed-state memoization: canonicalize by per-seat symmetry class so that
		// equivalent partial states (swapped identical snoids or empties) hit one entry.
		Common::String stateKey = canonicalStateKey();
		if (_failedStates.contains(stateKey))
			return false;

		int16 seat = selectNextSeat();
		if (seat < 0) {
			memoizeFailure(stateKey);
			return false;
		}

		int16 candidates[20] = {};
		int16 candidateCount = 0;
		buildOrderedCandidates(seat, candidates, candidateCount);

		for (int16 i = 0; i < candidateCount; i++) {
			int16 item = candidates[i];

			uint32 savedDomain[20];
			for (int16 k = 0; k < _seatCount; k++)
				savedDomain[k] = _domain[k];
			uint32 savedUnused = _unusedItemsMask;

			assignItem(seat, item);

			bool occupied = !isEmptyItem(item);
			bool ok = forwardCheckOk();
			if (ok && occupied)
				ok = neighborCapacityOk(seat, item);
			if (ok && search(assignedCount + 1))
				return true;

			for (int16 k = 0; k < _seatCount; k++)
				_domain[k] = savedDomain[k];
			_unusedItemsMask = savedUnused;
			_seatToItem[seat] = -1;
	...
	// Incremental propagation: commit an item to a seat and tighten the bitmask
	// domains of the still-unassigned seats instead of rebuilding candidate lists.
	void assignItem(int16 seat, int16 item) {
		_seatToItem[seat] = item;
		_unusedItemsMask &= ~(1u << item);

		uint32 clearBit = ~(1u << item);
		bool occupied = !isEmptyItem(item);
		uint32 neighborMask = occupied ? _neighborMask[item] : 0;
		uint32 seatNeighbors = _seatNeighborMask[seat];

		for (int16 nb = 0; nb < _seatCount; nb++) {
			if (_seatToItem[nb] != -1)
				continue;
			uint32 d = _domain[nb] & clearBit;
			if (occupied && ((seatNeighbors >> nb) & 1) != 0)
				d &= neighborMask;
			_domain[nb] = d;
		}
		_domain[seat] = 0;
	...
	bool forwardCheckOk() const {
		for (int16 seat = 0; seat < _seatCount; seat++) {
			if (_seatToItem[seat] == -1 && _domain[seat] == 0)
				return false;
		}
		return true;
	}

	// Necessary neighbor-capacity test: every still-empty neighbor of the seat we
	// just filled with an occupied snoid must receive an empty or a trait-sharing
	// snoid. If the unused compatible pool is smaller than that demand, prune now.
	bool neighborCapacityOk(int16 seat, int16 item) const {
		int16 needed = 0;
		uint32 seatNeighbors = _seatNeighborMask[seat];
		for (int16 nb = 0; nb < _seatCount; nb++) {
			if (((seatNeighbors >> nb) & 1) != 0 && _seatToItem[nb] == -1)
				needed++;
		}
		if (needed == 0)
			return true;

		int16 unusedEmpties = ferryCountBits(_unusedItemsMask & _emptyMask);
		int16 compatibleSnoids = ferryCountBits(_unusedItemsMask & _snoidMask & _compatSnoidMask[item]);
		return needed <= unusedEmpties + compatibleSnoids;
	}

	bool isBudgetExhausted() {
		if (_nodeCap <= _nodesVisited) {
			_timedOut = true;
			return true;
		}

		uint32 elapsedMs = _vm->_system->getMillis() - _startTime;
		if (_timeCapMs <= elapsedMs) {
			_timedOut = true;
			return true;
		}
	...
	int16 selectNextSeat() const {
		int16 bestSeat = -1;
		int16 bestCandidateCount = 100;
		int16 bestAssignedNeighborCount = -1;
		int16 bestDegree = -1;

		for (int16 seat = 0; seat < _seatCount; seat++) {
			if (_seatToItem[seat] != -1)
				continue;

			int16 localCandidateCount = candidateClassCount(seat);
			int16 assignedNeighborCount = countAssignedNeighbors(seat);
			int16 degree = countSeatDegree(seat);
			if (localCandidateCount < bestCandidateCount ||
					(localCandidateCount == bestCandidateCount && bestAssignedNeighborCount < assignedNeighborCount) ||
					(localCandidateCount == bestCandidateCount && bestAssignedNeighborCount == assignedNeighborCount && bestDegree < degree)) {
				bestSeat = seat;
				bestCandidateCount = localCandidateCount;
				bestAssignedNeighborCount = assignedNeighborCount;
				bestDegree = degree;
			}
		}
	...
	// MRV measured in distinct symmetry classes, matching the deduplicated branching.
	int16 candidateClassCount(int16 seat) const {
		uint32 seenClasses = 0;
		int16 count = 0;
		uint32 domain = _domain[seat];
		for (int16 item = 0; item < _itemCount; item++) {
			if (((domain >> item) & 1) == 0)
				continue;
			uint32 classBit = 1u << _classId[item];
			if ((seenClasses & classBit) != 0)
				continue;
			seenClasses |= classBit;
			count++;
		}
		return count;
	}

	void buildOrderedCandidates(int16 seat, int16 candidates[20], int16 &candidateCount) const {
		candidateCount = 0;
		uint32 seenClasses = 0;
		uint32 domain = _domain[seat];

		// One representative (lowest index) per symmetry class removes duplicate branches.
		for (int16 item = 0; item < _itemCount; item++) {
			if (((domain >> item) & 1) == 0)
				continue;
			uint32 classBit = 1u << _classId[item];
		...
		// Least-constraining-value ordering: try the item that preserves the most
		// neighbor freedom first, so a satisfiable pack reaches a solution sooner.
		int16 scores[20] = {};
		for (int16 i = 0; i < candidateCount; i++)
			scores[i] = leastConstrainingScore(seat, candidates[i]);

		for (int16 i = 1; i < candidateCount; i++) {
			int16 item = candidates[i];
			int16 score = scores[i];
			int16 j = i - 1;
	...
	int16 leastConstrainingScore(int16 seat, int16 item) const {
		int16 score = 0;
		bool occupied = !isEmptyItem(item);
		uint32 seatNeighbors = _seatNeighborMask[seat];
		uint32 clearBit = ~(1u << item);
		for (int16 nb = 0; nb < _seatCount; nb++) {
			...
			uint32 d = _domain[nb] & clearBit;
			if (occupied)
				d &= _neighborMask[item];
			score += ferryCountBits(d);
		}
		return score;
	}

	// Canonical key for memoization. Values start at 1 so the encoding never embeds a
	// NUL byte (Common::String hashing is NUL-terminated), and identical-trait snoids
	// or empties collapse onto the same class id.
	Common::String canonicalStateKey() const {
		char buffer[21];
		for (int16 seat = 0; seat < _seatCount; seat++) {
			int16 item = _seatToItem[seat];
			buffer[seat] = static_cast<char>(item == -1 ? 1 : (_classId[item] + 2));
		}
		return Common::String(buffer, _seatCount);
	}

	void memoizeFailure(const Common::String &stateKey) {
		if (_failedStates.size() < kFerryPerfectClearMemoCap)
			_failedStates[stateKey] = true;
	}

	bool isEmptyItem(int16 item) const {
		return _snoidCount <= item;
	}

	bool failsComponentBound() const {
		if (_emptyCount < 0)
			return true;
		return !feasibleComponentPartitionExists();
	}

	// Strengthened precheck. Every connected occupied-seat component must be filled by
	// snoids from a single compatibility component, so the occupied-seat-component sizes
	// must partition the compatibility-component sizes for at least one empty-seat layout.
	bool feasibleComponentPartitionExists() const {
		int16 compSizes[20] = {};
		int16 compCount = 0;
		computeCompatibilityComponentSizes(compSizes, compCount);

		if (_emptyCount == 0)
			return seatPartitionFeasible(0, compSizes, compCount);
		if (_seatCount <= _emptyCount)
			return _snoidCount == 0;

		return searchRemovalForPartition(0, _emptyCount, 0, compSizes, compCount);
	}

	bool searchRemovalForPartition(int16 startSeat, int16 seatsLeft, uint32 removedMask,
	                               const int16 compSizes[20], int16 compCount) const {
		if (seatsLeft == 0)
			return seatPartitionFeasible(removedMask, compSizes, compCount);

		for (int16 seat = startSeat; seat <= _seatCount - seatsLeft; seat++) {
			if (searchRemovalForPartition(seat + 1, seatsLeft - 1, removedMask | (1u << seat), compSizes, compCount))
				return true;
		}
		return false;
	}

	bool seatPartitionFeasible(uint32 removedMask, const int16 compSizes[20], int16 compCount) const {
		int16 seatSizes[20] = {};
		int16 seatCompCount = 0;
		computeSeatComponentSizes(removedMask, seatSizes, seatCompCount);

		if (compCount > seatCompCount)
			return false;

		// Sort occupied-seat-component sizes descending for tighter partition pruning.
		for (int16 i = 1; i < seatCompCount; i++) {
			int16 v = seatSizes[i];
			int16 j = i - 1;
			while (j >= 0 && seatSizes[j] < v) {
				seatSizes[j + 1] = seatSizes[j];
				j--;
			}
			seatSizes[j + 1] = v;
		}
		...
		int16 remaining[20] = {};
		for (int16 i = 0; i < compCount; i++)
			remaining[i] = compSizes[i];

		return assignSeatComponents(seatSizes, seatCompCount, 0, remaining, compCount);
	}

	bool assignSeatComponents(const int16 seatSizes[20], int16 seatCompCount, int16 index,
	                          int16 remaining[20], int16 compCount) const {
		if (index == seatCompCount) {
			for (int16 i = 0; i < compCount; i++) {
				if (remaining[i] != 0)
					return false;
			}
		...
		int16 size = seatSizes[index];
		for (int16 c = 0; c < compCount; c++) {
			if (remaining[c] < size)
				continue;

			// Skip compatibility components that currently have an identical remaining
			// capacity; they would only create symmetric, redundant assignments.
			bool duplicate = false;
			for (int16 prev = 0; prev < c; prev++) {
				if (remaining[prev] == remaining[c]) {
					duplicate = true;
					break;
				}
			}
			...
			remaining[c] -= size;
			if (assignSeatComponents(seatSizes, seatCompCount, index + 1, remaining, compCount))
				return true;
			remaining[c] += size;
		}
		return false;
	...
	void computeCompatibilityComponentSizes(int16 sizes[20], int16 &componentCount) const {
		...
		for (int16 i = 0; i < _snoidCount; i++) {
			...
			while (0 < stackSize) {
				int16 cur = stack[--stackSize];
				size++;
				for (int16 j = 0; j < _snoidCount; j++) {
					uint32 nextBit = 1u << j;
					if ((visitedMask & nextBit) != 0 || !_share[cur][j])
						continue;
					visitedMask |= nextBit;
					stack[stackSize++] = j;
				}
	...
	void computeSeatComponentSizes(uint32 removedMask, int16 sizes[20], int16 &componentCount) const {
		uint32 visitedMask = removedMask;
		componentCount = 0;
		...
		for (int16 seat = 0; seat < _seatCount; seat++) {
			...
			int16 stack[20] = {};
			int16 stackSize = 0;
			stack[stackSize++] = seat;
			visitedMask |= bit;

			while (0 < stackSize) {
				int16 cur = stack[--stackSize];
				size++;
				uint32 neighbors = _seatNeighborMask[cur];
				for (int16 nextSeat = 0; nextSeat < _seatCount; nextSeat++) {
					uint32 nextBit = 1u << nextSeat;
					if (((neighbors >> nextSeat) & 1) == 0 || (visitedMask & nextBit) != 0)
						continue;
					visitedMask |= nextBit;
					stack[stackSize++] = nextSeat;
				}
			}
	...
	int16 countAssignedNeighbors(int16 seat) const {
		int16 count = 0;
		uint32 neighbors = _seatNeighborMask[seat];
		for (int16 nb = 0; nb < _seatCount; nb++) {
			if (((neighbors >> nb) & 1) != 0 && _seatToItem[nb] != -1)
				count++;
		}
		return count;
	}

	int16 countSeatDegree(int16 seat) const {
		return ferryCountBits(_seatNeighborMask[seat]);
	}

	MohawkEngine_Zoombini *_vm;
	const byte (*_adjacencyMatrix)[8];
	const Common::Array<const ZmbSnoid *> &_snoids;
	int16 _seatCount = 0;
	int16 _snoidCount = 0;
	int16 _itemCount = 0;
	int16 _emptyCount = 0;
	...
	bool _share[20][20] = {};
	uint32 _seatNeighborMask[20] = {};
	uint32 _neighborMask[20] = {};
	uint32 _compatSnoidMask[20] = {};
	int16 _classId[20] = {};
	uint32 _snoidMask = 0;
	uint32 _emptyMask = 0;
	uint32 _allItemsMask = 0;
	uint32 _unusedItemsMask = 0;
	int16 _seatToItem[20] = {};
	uint32 _domain[20] = {};
	Common::HashMap<Common::String, bool> _failedStates;
	FerryPerfectClearResult _result;
};
...
Common::String ZoombiniPuzzleFerry::debugGetAnswer() const {
	return formatDebugAnswer(1);
}

Common::String ZoombiniPuzzleFerry::debugGetAnswerWithArgs(int argc, const char **argv) const {
	if (argc == 1)
		return formatDebugAnswer(1);

	if (argc != 2)
		return "Usage: printAnswer [positive-cap-multiplier]\n  Example: printAnswer 10\n";

	int32 parsedMultiplier = 0;
	if (!ZmbResource::parseInt(argv[1], parsedMultiplier) || parsedMultiplier < 1)
		return Common::String::format("Usage: printAnswer [positive-cap-multiplier]\n  Invalid cap multiplier: %s\n  Example: printAnswer 10\n", argv[1]);

	return formatDebugAnswer(static_cast<uint32>(parsedMultiplier));
}

Common::String ZoombiniPuzzleFerry::formatDebugAnswer(uint32 capMultiplier) const {
	...
	s += debugGetPerfectClearReport(capMultiplier, adjacencyMatrix);
...
Common::String ZoombiniPuzzleFerry::debugGetPerfectClearReport(uint32 capMultiplier, const byte adjacencyMatrix[20][8]) const {
	Common::Array<const ZmbSnoid *> snoids;
	for (auto it = _snoidMap.begin(); it != _snoidMap.end(); ++it) {
		const ZmbSnoid *snoid = *it;
		if (!snoid || !snoid->_trait.isComplete())
			continue;
		snoids.push_back(snoid);
	}

	Common::String s;
	if (_drawOnRegCount < 0 || 20 < _drawOnRegCount) {
		s += Common::String::format("  Perfect-clear check for current active pack: not checked (invalid seat count %d).\n", _drawOnRegCount);
		return s;
	}

	if (20 < snoids.size()) {
		s += Common::String::format("  Perfect-clear check for current active pack: not checked (too many snoids: %u).\n", static_cast<uint>(snoids.size()));
		return s;
	}

	if (static_cast<uint>(_drawOnRegCount) < snoids.size()) {
		s += Common::String::format("  Perfect-clear check for current active pack: impossible (snoids=%u seats=%d).\n",
			static_cast<uint>(snoids.size()), _drawOnRegCount);
		return s;
	}

	uint32 nodeMultiplier = capMultiplier;
	if (UINT32_MAX / kFerryDefaultPerfectClearNodeCap < nodeMultiplier)
		nodeMultiplier = UINT32_MAX / kFerryDefaultPerfectClearNodeCap;
	uint32 timeMultiplier = capMultiplier;
	if (UINT32_MAX / kFerryDefaultPerfectClearTimeCapMs < timeMultiplier)
		timeMultiplier = UINT32_MAX / kFerryDefaultPerfectClearTimeCapMs;

	uint32 nodeCap = kFerryDefaultPerfectClearNodeCap * nodeMultiplier;
	uint32 timeCapMs = kFerryDefaultPerfectClearTimeCapMs * timeMultiplier;

	FerryPerfectClearSolver solver(_vm, adjacencyMatrix, snoids, _drawOnRegCount, nodeCap, timeCapMs);
	FerryPerfectClearResult result = solver.run();

	if (result._status == kFerryPerfectClearPossible) {
		s += Common::String::format("  Perfect-clear check for current active pack: possible (snoids=%u seats=%d, searched=%u nodes/%u ms; cap=%u nodes/%u ms).\n",
			static_cast<uint>(snoids.size()), _drawOnRegCount, result._nodesVisited, result._elapsedMs, nodeCap, timeCapMs);
		s += "  Perfect-clear assignment:\n";
		for (int16 seat = 0; seat < _drawOnRegCount; seat++) {
			if (seat % 4 == 0)
				s += "    ";

			int16 item = result._seatToItem[seat];
			if (0 <= item && static_cast<uint>(item) < snoids.size())
				s += Common::String::format("seat %d=snoid %u", seat, static_cast<uint>(snoids[item]->getId()));
			else
				s += Common::String::format("seat %d=empty", seat);

			if (seat % 4 == 3 || seat + 1 == _drawOnRegCount)
				s += "\n";
			else
				s += ", ";
		}
	} else if (result._status == kFerryPerfectClearImpossible) {
		s += Common::String::format("  Perfect-clear check for current active pack: impossible (exact; snoids=%u seats=%d, searched=%u nodes/%u ms; cap=%u nodes/%u ms).\n",
			static_cast<uint>(snoids.size()), _drawOnRegCount, result._nodesVisited, result._elapsedMs, nodeCap, timeCapMs);
	} else {
		s += Common::String::format("  Perfect-clear check for current active pack: unknown(timeout) (snoids=%u seats=%d, searched=%u nodes/%u ms; cap=%u nodes/%u ms).\n",
			static_cast<uint>(snoids.size()), _drawOnRegCount, result._nodesVisited, result._elapsedMs, nodeCap, timeCapMs);
		s += "  For a deeper search, try printAnswer <cap-multiplier> (for example, printAnswer 10).\n";
	}
